...
/**
 *
 * @param {readonly [remoteUrl: string, diskPath:string, mapper?: (dlData: string) => string]} param0
 */
async function downloadAndSaveFile([remoteUrl, diskPath, mapper = String]) {
  const file = await (await fetch(new URL(baseUrl + remoteUrl))).text();

  const pathToSave = url.fileURLToPath(
    path.join(import.meta.url, "../", diskPath)
  );
  ...
  await fs.writeFile(pathToSave, mapper(file), {});
}
...
const files = /** @type {const} */ ([
  [
    ...
    "./monaco-editor.d.ts",
    (file) => `declare module "monaco-editor" {\n${file}\n}`,
  ],
  ...
  [
    ...
    "./public/vs/editor/editor.main.nls.js",
  ],
  [
  ...
  [
    ...
    "./public/vs/language/typescript/tsMode.js",
  ],
  [
    "/min/vs/base/worker/workerMain.js",
    "./public/vs/base/worker/workerMain.js",
  ],
  [
...
export const download = async (force = false) => {
  if (
    ...
    return void Promise.all(files.map(downloadAndSaveFile));
  }
};
